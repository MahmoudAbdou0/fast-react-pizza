...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    small: base + 'px-2 py-2 md:px-5 md:py-2.5 text-sm',
    round: base + 'px-2.5 py-1 md:px-3.5 md:py-2 text-sm',
    secondary:
      'text-sm inline-block rounded-full border-2 border-stone-300 font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 hover:text-stone-800 focus:text-stone-800 hover:bg-stone-300 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed px-4 py-2.5 md:px-6 md:py-3.5',
  };
  if (to)
    return (
